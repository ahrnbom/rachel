...
{
    return std::chrono::duration_cast<TimeDelta>(
...
/*
    Obtains the current time, taking into account if the system is running live
   or from a recording.
*/
Time current_time();

/*
    Set to true when it is time to shut down the program. All nodes are expected
   to quickly turn themselves off, which can be achieved by regularly using
   main_loop_condition. It is marked as `extern` so that all nodes share the
   same object, and it is atomic for thread safety. The actual value is defined
   in rachel.cpp
*/
...
public:
    ...
    /*
        Subscribe by value, which means that the data pointer will be regularly
    ...
    */
    ...
    /*
        Queue based subscription, which means that the callback will be
       regularly called when new values are published.
    */
    template <typename T>
...
/*
    Starts all node threads. Waits for all nodes to finish. Typically called
